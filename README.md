# My studies

In this repository are listed other repositories with examples of training and experiments of all the technologies that I use at work or find interesting.

## PHP

- [New in PHP 8.1 - Study](https://github.com/danie1net0-labs/php-8.1)
- [Laravel Jobs - Study](https://github.com/danie1net0-labs/laravel-jobs)
- [Iterators - Study](https://github.com/danie1net0-labs/php-iterators)
- [Generators - Study](https://github.com/danie1net0-labs/php-generators)
- [Reactive PHP - Alura trainning](https://github.com/danie1net0-labs/alura-reactive-php)
- [Solid - Alura trainning](https://github.com/danie1net0-labs/alura-solid-php)
- [Object calisthenics - Alura trainning](https://github.com/danie1net0-labs/alura-object-calisthenics)
- [Behavioral Patterns - Alura trainning](https://github.com/danie1net0-labs/alura-behavioral-patterns)
- [Knowing Doctrine ORM - Alura trainning](https://github.com/danie1net0/alura-knowing-docrine-orm)
- [Doctrine: migrations, reports and performance - Alura trainning](https://github.com/danie1net0-labs/alura-doctrine-migrations-and-cache)

## JavaScript

- [Creating a library with NodeJS - Alura trainning](https://github.com/danie1net0-labs/alura-lib-nodejs)

## Infra
- [AWS Lambda - Alura trainning](https://github.com/danie1net0-labs/alura-aws-lambda)
